...
This repository preserves the upstream history and license of SWATrunR. Package names remain unchanged so existing R scripts can continue using `library(SWATrunR)`. This is a tested development update, not an upstream release.

Upstream starting commit: `c66a60eb767d5b8c8f1790959a195cbd08a97259`. SWATrunR uses upstream `remove_legacy` (1.1.0.9017), matching the bundled workflow; its one local difference was portable library propagation to workers, which is retained in this update.
